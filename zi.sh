...
# - BusyBox/Android console compatible (no read -s)
# - Protects against dpkg/apt locks and venv issues
# - Auto-truncates admin password to bcrypt 72 bytes limit
# - Auto-creates admin account, shows creds, auto-deletes creds file after 24h
...
# -------------------------
# Helper: safe apt operations (remove locks, fix dpkg)
safe_apt_prepare() {
...
# -------------------------
# Prompt admin creds (plain read for BusyBox)
...
# -------------------------
# Protect against very long/Unicode password: truncate to 72 bytes (bcrypt limit)
# This keeps utf-8 validity as best-effort (drops incomplete tail bytes)
ADMIN_PASS_TRUNC=$(
python3 - <<PY
...
echo "[*] (password shown only once at end; also saved temporarily in /etc/zivpn/admin_creds.txt)"
...
# Another protection: ensure python3-venv and distutils available
safe_apt_prepare
...
if wget -q --inet4-only -O /usr/local/bin/zivpn "$ZIVPN_BIN_URL"; then
  chmod +x /usr/local/bin/zivpn
else
  echo "[!] Failed to download zivpn binary from GitHub (network). Exiting."
  exit 1
fi
mkdir -p /etc/zivpn
...
# Ensure venv works: try install python3-venv and distutils if missing
safe_apt_prepare
apt-get install -y python3-venv python3-distutils || true

python3 -m venv "$ADMIN_DIR/venv" || { echo "[!] venv creation failed, trying to repair apt/dpkg and retry..."; safe_apt_prepare; apt-get install -y python3-venv python3-distutils; python3 -m venv "$ADMIN_DIR/venv"; }
...
# Initialize schema and insert admin
sqlite3 "$DB_PATH" <<SQL
