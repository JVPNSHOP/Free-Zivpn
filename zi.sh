...
if [ -n "$input_config" ]; then
    # remove spaces around commas to keep clean
    cleaned=$(echo "$input_config" | tr -d ' ')
    IFS=',' read -r -a config <<< "$cleaned"
    if [ ${#config[@]} -eq 1 ]; then
        config+=("${config[0]}")
    fi
else
...
# --- REPLACED: safe merge using Python to avoid breaking JSON or overwriting existing passwords ---
# Build a comma separated string to pass into python env
PASSES=$(printf "%s," "${config[@]}" | sed 's/,$//')
export PASSES
...
if not isinstance(existing, list):
    existing = []
# get incoming from env
incoming = os.environ.get("PASSES", "")
cands = [p for p in incoming.split(",") if p]
...
print("Updated /etc/zivpn/config.json with", existing)
PY
# --- end replacement ---
...
if [ -n "$IFACE" ]; then
  # Use --to-ports for DNAT to local port (more correct for mapping ranges to single port)
  iptables -t nat -A PREROUTING -i "${IFACE}" -p udp --dport 6000:19999 -j DNAT --to-ports 5667 2> /dev/null || true
fi
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    /* ensure alert action buttons are visible on both dark and light theme */
    ...
    /* small icon styling for server overview (keeps spacing neat) */
    .stat-ic{font-size:16px;opacity:0.95}
    /* server ip box style (so copy visible) */
...
function copyToClipboard(text){
  // Trim to avoid weird whitespace
