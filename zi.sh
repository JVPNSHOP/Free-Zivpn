#!/bin/bash
# install-zivpn-fixed-no-signup.sh
# ZIVPN UDP + Admin Panel installer (NO SIGNUP; installer-provided admin shown permanently on login UI)
# - No signup route (admin only)
# - Admin creds entered at install-time and saved to /etc/zivpn/admin_creds.txt (NOT auto-deleted)
# - bcrypt 72-byte safe (utf-8 truncate)
# - Stops existing zivpn before download (avoid "Text file busy")
# - BusyBox/Android compatible (no read -s)
# Run as root

set -e
export DEBIAN_FRONTEND=noninteractive

echo "=== ZIVPN INSTALLER (NO SIGNUP, ADMIN PROVIDED) ==="
...
echo "[*] Using admin username: $ADMIN_USER"
echo "[*] (password displayed at end and stored in /etc/zivpn/admin_creds.txt)"
...
if [ ! -f /usr/local/bin/zivpn ]; then
  echo "[!] Failed to download zivpn binary. Exiting."
  exit 1
fi
...
# initial config passwords (optional)
...
# -------------------------
# Install Flask Admin Panel (NO SIGNUP)
...
if ! python3 -m venv "$ADMIN_DIR/venv"; then
  echo "[!] venv creation failed; attempting repair and retry..."
  safe_apt_prepare
  apt-get install -y python3-venv python3-distutils || true
...
# Save admin creds permanently (user requested) — DO NOT auto-delete
...
# Initialize DB and insert admin user (no signup route)
sqlite3 "$DB_PATH" <<SQL
...
INSERT OR REPLACE INTO users (id, username, password_hash, created_at)
  VALUES ((SELECT id FROM users WHERE username='${ADMIN_USER}'), '${ADMIN_USER}', '${HASHED_PASS}', datetime('now'));
SQL

# Write Flask app WITHOUT signup route
...
# SIGNUP REMOVED - admin accounts must be created during install or via DB manually
...
def login():
    ...
    if request.method=="POST":
        ...
        if row and bcrypt.verify(password, row['password_hash']):
            ...
            return redirect(url_for('login'))
    # Always show the installer-provided admin creds (persisted)
...
# Templates (login without signup, other pages kept)
...
# login.html (always shows admin creds from /etc/zivpn/admin_creds.txt)
cat > "$TEMPL_DIR/login.html" <<'HTM'
{% extends "base.html" %}
{% block content %}
{% if admin_creds %}
  <div class="creds">
    <strong>Panel Login (use below credentials)</strong><br>
    Username: <code>{{ admin_creds.get('username') }}</code> &nbsp;
    Password: <code>{{ admin_creds.get('password') }}</code>
    <div class="small">Use these credentials to login. Signup is disabled on this panel.</div>
  </div>
{% endif %}
<div class="card">
  <h3>Login</h3>
...
# keep other templates (dashboard/create_password/success/accounts/contact/donate) — same as usual
...
# daily cleanup timer for expired VPN passwords
...
echo
echo "• Signup is disabled on this panel. Use the credentials above to login."
echo "• Admin credentials are stored at /etc/zivpn/admin_creds.txt (visible on login UI)."
echo "• To change admin password later, login and update DB or run a hash/UPDATE on /etc/zivpn/admin.sqlite."
echo "=============================================="
