...
# Prompt for initial ZIVPN passwords
echo -e "\n=== ZIVPN UDP Passwords ==="
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    [data-theme="light"] input,[data-theme="light"] select{background:#f9fafb;}
    ...
    .alert-actions button{flex:1;padding:8px;font-size:12px;background:rgba(15,23,42,.9);border-radius:999px;border:1px solid rgba(148,163,184,.5)}
    [data-theme="light"] .alert-actions button{background:#f9fafb;}
  ...
  <div class="container">
    <div class="left-col">
      <div class="card" role="region" aria-label="Server Overview">
        ...
        <div class="stat-row">
          <div class="stat-label"><span class="dot dot-blue"></span>Server Time</div>
          <div class="stat-value">{{ stats.time }}</div>
        </div>
        <div class="stat-row">
          <div class="stat-label"><span class="dot dot-green"></span>CPU Load</div>
          <div class="stat-value">{{ stats.load }}</div>
        </div>
        <div class="stat-row">
          <div class="stat-label"><span class="dot dot-purple"></span>RAM Usage</div>
          <div class="stat-value">{{ stats.mem }}</div>
        </div>
        <div class="stat-row">
          <div class="stat-label"><span class="dot dot-orange"></span>Storage</div>
          <div class="stat-value">{{ stats.disk }}</div>
        </div>
        <div class="stat-row">
          <div class="stat-label"><span class="dot dot-pink"></span>Total Account</div>
          <div class="stat-value" id="totalAccount">{{ total }}</div>
        </div>
        <div class="stat-row" style="border-bottom:none">
          <div class="stat-label"><span class="dot dot-yellow"></span>Total Offline</div>
    ...
    <div class="right-col">
      <aside class="card" role="complementary">
        ...
        <div style="display:flex;gap:8px;margin-top:6px;align-items:center">
          <div id="serverIpBox" style="flex:1;padding:10px;border-radius:12px;background:rgba(15,23,42,.75);border:1px solid rgba(148,163,184,.4)">{{ server_ip }}</div>
          <button style="width:auto;padding:8px 12px;border-radius:999px;font-size:12px" onclick="copyToClipboard(document.getElementById('serverIpBox').innerText)">Copy</button>
        </div>
...
document.getElementById('addBtn').addEventListener('click', async ()=>{
  ...
  if(!pwd){
    const a=document.getElementById('alerts');
    const el=document.createElement('div'); el.className='alert-card'; el.style.borderColor='rgba(248,113,113,.6)'; el.style.background='rgba(248,113,113,.12)'; el.innerText='Password required';
  ...
  try{
    ...
    if(j.ok){
      ...
      el.querySelectorAll('[data-copy]').forEach(btn=>{
        btn.addEventListener('click',()=>{navigator.clipboard.writeText(btn.getAttribute('data-copy'));btn.innerText='Copied';setTimeout(()=>btn.innerText='Copy',1200);});
      });
      ...
      const a=document.getElementById('alerts'); const el=document.createElement('div'); el.className='alert-card'; el.style.borderColor='rgba(248,113,113,.6)'; el.style.background='rgba(248,113,113,.12)'; el.innerText='Error: '+(j.error||'unknown'); a.prepend(el); setTimeout(()=>el.remove(),5000);
    }
  }catch(e){
    const a=document.getElementById('alerts'); const el=document.createElement('div'); el.className='alert-card'; el.style.borderColor='rgba(248,113,113,.6)'; el.style.background='rgba(248,113,113,.12)'; el.innerText='Request failed'; a.prepend(el); setTimeout(()=>el.remove(),5000);
  }
});
