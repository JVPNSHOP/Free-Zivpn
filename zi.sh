# save as install-zivpn-full.sh, then: sudo bash install-zivpn-full.sh
#!/bin/bash
# Complete ZIVPN UDP + Admin Panel Installer
# - installs zivpn binary
# - installs Flask admin panel (with admin creds created at install-time)
# - shows creds on login UI & in terminal (one-time)
# - auto-deletes creds file after 24h via systemd timer
# - nginx + Let's Encrypt (optional, requires domain)
# - Create password expiry = 5 days, JS popup + copy buttons
# RUN AS ROOT

set -e
export DEBIAN_FRONTEND=noninteractive

echo "== ZIVPN Full Installer =="
...
read -p "Provide domain for HTTPS (leave blank to skip nginx/Let's Encrypt): " DOMAIN
if [ -n "$DOMAIN" ]; then
  read -p "Email for Let's Encrypt notifications (required for cert): " LE_EMAIL
...
wget -q https://github.com/zahidbd2/udp-zivpn/releases/download/udp-zivpn_1.4.9/udp-zivpn-linux-amd64 -O /usr/local/bin/zivpn || { echo "Failed to download zivpn"; exit 1; }
chmod +x /usr/local/bin/zivpn
mkdir -p /etc/zivpn
wget -q https://raw.githubusercontent.com/zahidbd2/udp-zivpn/main/config.json -O /etc/zivpn/config.json || true

# certs for zivpn (self-signed)
...
# prompt initial zivpn config passwords (optional)
...
apt-get install -y python3 python3-venv python3-pip sqlite3 nginx >/dev/null 2>&1 || true
...
# save creds file (shown on login UI); will be auto-deleted by timer after 24h
CREDS_FILE="/etc/zivpn/admin_creds.txt"
echo "username=${ADMIN_USER}" > "$CREDS_FILE"
echo "password=${ADMIN_PASS}" >> "$CREDS_FILE"
chmod 600 "$CREDS_FILE"

# create hashed password using venv python
HASHED_PASS=$("$ADMIN_DIR/venv/bin/python" - <<PY
from passlib.hash import bcrypt
print(bcrypt.hash("${ADMIN_PASS}"))
PY
)

# initialize DB tables and insert admin user (if not exists)
sqlite3 "$DB_PATH" <<SQL
...
def create_password():
    if request.method=="POST":
        ...
        expire = created + datetime.timedelta(days=5)
        db = get_db()
...
# systemd service for admin panel (Flask)
...
# 3) Setup creds auto-delete timer (24h)
cat >/etc/systemd/system/zivpn-clear-creds.service <<'CLR'
[Unit]
Description=Remove ZIVPN install-created creds file
...
# 4) cleanup expired timer daily (call internal endpoint)
...
# 5) nginx + certbot (optional)
if [ -n "$DOMAIN" ]; then
  echo "Configuring nginx and obtaining Let's Encrypt certificate for $DOMAIN..."
  apt-get install -y certbot python3-certbot-nginx >/dev/null 2>&1 || true

  # create nginx site config to proxy to flask
...
NG
  sed -i "s/DOMAIN_PLACEHOLDER/${DOMAIN}/g" /etc/nginx/sites-available/zivpn
  ln -sf /etc/nginx/sites-available/zivpn /etc/nginx/sites-enabled/zivpn
  rm -f /etc/nginx/sites-enabled/default
  systemctl restart nginx || true

  # obtain cert (requires domain pointed to this server and port 80 open)
  certbot --nginx -n --agree-tos --email "$LE_EMAIL" -d "$DOMAIN" || {
    echo "certbot failed. Check domain DNS and port 80 accessibility. You can retry manually later."
  }
fi

# final info print (one-time show)
...
echo "=============================================="
if [ -n "$DOMAIN" ]; then
  echo " Panel URL     : https://$DOMAIN (nginx & certbot configured)"
else
  echo " Panel URL     : http://$SERVER_IP:8000"
fi
echo " Admin Username: ${ADMIN_USER}"
echo " Admin Password: ${ADMIN_PASS}"
echo " NOTE: Credentials also shown on Login UI (if /etc/zivpn/admin_creds.txt exists)."
echo " The creds file will be auto-deleted in 24 hours (or remove it manually after first login)."
echo " To remove now: sudo rm -f /etc/zivpn/admin_creds.txt && sudo systemctl restart zivpn-admin.service"
echo "=============================================="
