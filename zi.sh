...
# Compatible with BusyBox/Android terminal (no read -s)
# RUN AS ROOT
...
# 0) Prompt admin creds (plain read to avoid BusyBox read -s issues)
...
if [ -n "$DOMAIN" ]; then
  read -p "Email for Let's Encrypt (required for certs) [default: admin@$DOMAIN]: " LE_EMAIL
...
wget -q "$ZIVPN_BIN_URL" -O /usr/local/bin/zivpn || { echo "Failed to download zivpn binary"; exit 1; }
chmod +x /usr/local/bin/zivpn
mkdir -p /etc/zivpn
wget -q https://raw.githubusercontent.com/zahidbd2/udp-zivpn/main/config.json -O /etc/zivpn/config.json || true

# 1a) Self-signed certs for zivpn (if not present)
...
# 1b) Kernel tuning (best effort)
sysctl -w net.core.rmem_max=16777216 >/dev/null 2>&1 || true
sysctl -w net.core.wmem_max=16777216 >/dev/null 2>&1 || true

# 1c) Systemd service for zivpn
...
# 3) Prompt optional initial zivpn passwords (same original behavior)
...
apt-get install -y python3 python3-venv python3-pip sqlite3 nginx certbot python3-certbot-nginx >/dev/null 2>&1 || true
...
# Save credentials in a file (shown on login UI). We'll auto-delete later via timer.
CREDS_FILE="/etc/zivpn/admin_creds.txt"
echo "username=${ADMIN_USER}" > "$CREDS_FILE"
echo "password=${ADMIN_PASS}" >> "$CREDS_FILE"
chmod 600 "$CREDS_FILE"

# Generate hashed password using venv python
...
# Initialize DB schema and insert admin user (if not exists)
sqlite3 "$DB_PATH" <<SQL
...
if __name__ == "__main__":
    # bind to localhost to be reverse proxied by nginx; if you want public direct, change host to 0.0.0.0
    app.run(host="127.0.0.1", port=8000)
PY

# 4b) Templates (Login, Signup, Dashboard, Create Password, Success, Accounts, Contact, Donate)
...
# 4c) systemd service for admin panel
...
# 7) nginx & Let's Encrypt (optional)
if [ -n "$DOMAIN" ]; then
  echo "Configuring nginx for domain: $DOMAIN"
  # simple nginx proxy config
...
NG
  ...
  echo "If certbot succeeded, panel is available at https://$DOMAIN"
fi
...
echo
echo "• Credentials are also shown on the Login page while /etc/zivpn/admin_creds.txt exists."
echo "• The creds file will be auto-deleted after 24 hours. Remove manually after first login:"
