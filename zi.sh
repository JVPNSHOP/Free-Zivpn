#!/bin/bash
# zi.sh - ZIVPN UDP installer + Admin Web Panel (full UI templates included)
# Run as root: sudo bash zi.sh
set -euo pipefail
...
echo -e "\n=== Installing udp-zivpn binary (if missing) ==="
...
echo -e "\n=== Ensure default /etc/zivpn/config.json exists ==="
...
[ ${#config[@]} -eq 0 ] && config=("zi")

ensure_python3

# Atomically update /etc/zivpn/config.json auth.config
...
cfg["auth"] = auth
# tidy legacy top-level config if duplicated
...
python3 /tmp/update_zivpn_config.py >/dev/null 2>&1 || true
...
# iptables redirect for UDP range -> port 5667
IFACE=$(ip -4 route ls | awk '/default/ {for(i=1;i<=NF;i++){ if($i=="dev"){ print $(i+1); exit }}}' | head -1 || true)
if [ -z "$IFACE" ]; then IFACE="eth0"; fi
echo "Using interface: $IFACE"
if ! iptables -t nat -C PREROUTING -i "${IFACE}" -p udp --dport 6000:19999 -j REDIRECT --to-ports 5667 >/dev/null 2>&1; then
  iptables -t nat -A PREROUTING -i "${IFACE}" -p udp --dport 6000:19999 -j REDIRECT --to-ports 5667 >/dev/null 2>&1 || true
fi
...
echo -e "\n=== ZIVPN UDP Installed (port 5667) ==="

##########################################
# Admin Web Panel (Flask, HTTP:8000) - full UI templates
##########################################

echo -e "\n=== Installing Python venv + Flask (if missing) ==="
...
document.getElementById('addBtn').addEventListener('click', async ()=>{
  ...
  try{
    const res = await fetch('/api/add',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:pwd,days:days}),credentials:'same-origin'});
...
async function refreshStats(){
  try{
    ...
    if(j.ok){
      const s = j.stats || {};
      document.getElementById('stat_time').innerText = s.time || '';
      document.getElementById('stat_load').innerText = s.load || '';
      document.getElementById('stat_mem').innerText = s.mem || '';
      document.getElementById('stat_disk').innerText = s.disk || '';
      document.getElementById('totalAccount').innerText=j.total;
      document.getElementById('totalOffline').innerText=j.expired;
      document.getElementById('dailyCount').innerText=(j.daily&&j.daily.count)?j.daily.count:0;
      document.getElementById('quotaText').innerText=(j.daily&&j.daily.count)?j.daily.count+' / 250':'0 / 250';
      document.getElementById('serverIpBox').innerText=j.ip || document.getElementById('serverIpBox').innerText;
    }
  }catch(e){}
}
...
echo "Panel logs: sudo journalctl -u zivpn-panel.service -n 200 --no-pager"
