#!/bin/bash
# zi.sh - ZIVPN UDP installer + Admin Web Panel (fixed: auth.config used, no duplicate top-level config)
set -e
...
try:
    ...
    cfg = {}
# ensure auth exists and is a dict
auth = cfg.get("auth", {})
...
if "config" in cfg and not isinstance(cfg.get("auth"), dict):
    del cfg["config"]
# write atomically
tmp = p + ".tmp"
...
if [ -n "$IFACE" ]; then
  ...
  # Avoid duplicate rule: check if similar rule exists first
...
# Flask app (updated to modify auth.config correctly)
