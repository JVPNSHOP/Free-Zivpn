#!/bin/bash
# zi.sh - ZIVPN UDP installer + Admin Web Panel (Add-only, Menu, Dark/Light, 3D running text)
...
# - Add-only: /api/add endpoint (no bulk)
# - Menu: Donate, Contact Admin, Dark/Light
# - Footer 3D running text (marquee)
##########################################
...
# Create templates and static files (final UI: no Quick Actions, no Bulk, Add-only, Menu, Dark/Light, footer 3D running text)
...
<head>
  ...
  <style>
    :root{--bg:#0f172a;--card:#071028;--accent:#fb7185;--muted:#94a3b8;--text:#fff}
    ...
    input{width:100%;padding:12px;margin:8px 0;border-radius:8px;border:1px solid rgba(255,255,255,.04);background:transparent;color:var(--text);box-sizing:border-box}
...
<head>
  ...
  <style>
    :root{
      --bg:#0f172a; --card:#071028; --accent:#fb7185; --muted:#94a3b8; --text:#fff;
      --glass: rgba(255,255,255,0.02);
    }
    [data-theme="light"]{
      --bg:#f7fafc; --card:#ffffff; --accent:#ef5b8c; --muted:#475569; --text:#0b1220; --glass: rgba(11,17,32,0.04);
    }
    *{box-sizing:border-box}
    ...
    header{display:flex;justify-content:space-between;align-items:center;padding:12px 16px;border-bottom:1px solid rgba(0,0,0,0.06);background:linear-gradient(180deg,rgba(11,17,32,.9),rgba(11,17,32,.6));backdrop-filter: blur(4px);position:sticky;top:0;z-index:20}
    .brand{font-weight:800;font-size:18px}
    .menu-btn{background:transparent;border:1px solid rgba(255,255,255,.04);padding:8px 12px;border-radius:10px;color:var(--text);cursor:pointer}
    .dropdown{position:absolute;right:16px;top:56px;background:var(--card);padding:10px;border-radius:10px;box-shadow:0 6px 20px rgba(0,0,0,.4);display:none}
    .dropdown a{display:block;padding:8px 10px;text-decoration:none;color:var(--text);border-radius:8px}
    .container{padding:18px;display:grid;grid-template-columns:1fr 360px;gap:18px;flex:1}
    ...
    /* 3D running text */
    .marquee-wrap{perspective:600px;overflow:hidden}
    .marquee{display:inline-block;padding:12px 24px;border-radius:10px;background:linear-gradient(90deg,#111827,#0b1220);transform:translateZ(0);animation:marquee 18s linear infinite;white-space:nowrap;font-weight:700}
    @keyframes marquee{0%{transform:translateX(100%) rotateX(20deg)}100%{transform:translateX(-100%) rotateX(20deg)}}
  </style>
</head>
  ...
  <header>
    ...
    <div style="position:relative">
      ...
      <div id="dropdown" class="dropdown" role="menu" aria-hidden="true">
        ...
        <a href="#" id="contactLink">Contact Admin</a>
        <a href="#" id="themeToggle">Switch to Light Mode</a>
      </div>
    </div>
  </header>
    ...
    <div class="right-col">
      <aside class="card">
        ...
        <div style="display:flex;gap:8px;margin-top:6px;align-items:center">
          ...
          <div style="cursor:pointer;padding:8px;border-radius:8px;background:var(--card)" onclick="copyToClipboard(document.getElementById('serverIpBox').innerText)">Copy</div>
        </div>
...
function applyTheme(t){
  if(t === 'light'){ body.setAttribute('data-theme','light'); themeToggle.innerText='Switch to Dark Mode'; }
  else { body.setAttribute('data-theme','dark'); themeToggle.innerText='Switch to Light Mode'; }
...
document.getElementById('addBtn').addEventListener('click', async ()=>{
  ...
  if(!pwd){ const a=document.getElementById('alerts'); a.prepend(Object.assign(document.createElement('div'),{className:'alert-card',innerText:'Password required'})); setTimeout(()=>a.firstChild && a.removeChild(a.firstChild),3000); return; }
  try{
    ...
    if(j.ok){
      ...
      const a=document.getElementById('alerts'); a.prepend(Object.assign(document.createElement('div'),{className:'alert-card',innerText:'Error: '+(j.error||'unknown')})); setTimeout(()=>a.firstChild && a.removeChild(a.firstChild),5000);
    }
  } catch(e){
    const a=document.getElementById('alerts'); a.prepend(Object.assign(document.createElement('div'),{className:'alert-card',innerText:'Request failed'})); setTimeout(()=>a.firstChild && a.removeChild(a.firstChild),5000);
  }
});
...
echo -e "\nMenu (top-right): Donate | Contact Admin | Dark/Light"
